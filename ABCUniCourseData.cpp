...
public:
    ...
    void inject(const std::vector<Course*>& newCourses) {
        ...
        for (const Course* course : newCourses) {
            ...
            // Create new node
            DataNode* newNode = new DataNode(const_cast<Course*>(course));
...
public:
    ...
    static void load(DataStructure& dataStruct, const std::string& fileLocation) {
        GUI::clearScreen();
        std::cout << "Loading data from: " << fileLocation << std::endl;

        // Get a String list of the lines from the input file
        FileReader::readFile(dataStruct, fileLocation);
    }
...
int main() {
    ...
    std::string sourceFile = "courses.csv";
    ...
    do {
        ...
        switch (choice) {
            case 1:
                Menu::load(courseList, sourceFile);
                GUI::waitForInput();
                GUI::clearScreen();
                break;
